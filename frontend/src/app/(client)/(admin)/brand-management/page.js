...
import Table from "@/components/admin/brand/Table";
import Pagination from "@/components/admin/brand/Pagination";
...
const brandManagementPage = () => {
  ...
  const loadBrands = async (reset = false) => {
    const res = await fetchAllBrands({ page, search });

    if (res) {
      if (reset) {
        setBrandData(res);
      } else {
        setBrandData((prev) => [...prev, ...res.data]); 
      }
    }
  };
        ...
        <div className=" mx-auto max-w-screen-2xl">
          <div className="relative overflow-hidde sm:rounded-lg">
            <div className="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
              <div className="flex items-center flex-1 space-x-4">
                <h5>
                  ...
                  <span className="dark:text-white">{brandData.total}</span>
                </h5>
              </div>
              <div className="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                <div className="border border-gray-100/8 hover:border-primary p-1.5 rounded-lg">
                  <Link className=" flex items-center  gap-2" href={"/brand-management/add"}>
            ...
            <div className="overflow-x-auto">
              <Table brands={brandData?.brands} />
            </div>
            <Pagination page={page} setPage={setPage} overAllTotal={brandData?.total} LIMIT={limit} />
          </div>
        </div>
      </section>
    </>
