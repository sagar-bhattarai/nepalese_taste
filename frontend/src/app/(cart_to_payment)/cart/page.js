...
import { decreaseQuantity, removeFromCart, increaseQuantity, clearCart } from "../../../redux/cart/cartSlice";
import { useRouter } from "next/navigation";
import { CHECKOUT_ROUTE, HOME_ROUTE, PRODUCTS_ROUTE } from "@/constants/routes";
import Link from "next/link";

const CartPage = () => {
  const router = useRouter();
  const dispatch = useDispatch();
  const { products, totalPrice, discount, tax, finalPrice, totalCount } = useSelector((state) => state.cart);
  ...
  const increaseQuantityHandle = (product) => {
    dispatch(increaseQuantity(product));
  };

  const decreaseQuantityHandle = (product) => {
      ...
      <div className="max-w-5xl max-lg:max-w-2xl mx-auto p-4">
        ...
        <div className="grid lg:grid-cols-3 lg:gap-x-8 gap-x-6 gap-y-8 mt-6">
          ...
          {products.length != 0
                ...
                <div key={product._id} className="flex gap-4 bg-[#ffffff38] dark:bg-[#2e2e2e4a] px-4 py-6 rounded-md shadow-sm border border-gray-200 dark:border-[#e5e7eb2e]">
                  <div className="flex gap-6 sm:gap-4 max-sm:flex-col">
                    ...
                    <div className="flex flex-col gap-4">
                      <div>
                        <Link href={`/products/${product._id}`}>
                          <h3 className="hover:underline text-sm sm:text-base font-semibold text-slate-900 dark:text-slate-500">{product.productName}</h3>
                        </Link>
                  ...
                  <div className="ml-auto flex flex-col">
                    <div className="flex items-start gap-4 justify-end items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" className=" mt-1 w-4 h-4 cursor-pointer fill-slate-400 hover:fill-pink-600 inline-block" viewBox="0 0 64 64">
                    ...
                    <div className="flex items-center gap-3 mt-auto">
                      <button type="button"
                        onClick={() => increaseQuantityHandle(product)}
                      ...
                      <button type="button"
                        ...
                        onClick={() => decreaseQuantityHandle(product)}
            ...
            :
            <div className="flex items-center flex-col justify-center text-center text-sm leading-6 bg-[#ffffff38] dark:bg-[#2e2e2e4a] rounded-md lg:col-span-2  shadow-sm border border-gray-200 dark:border-[#e5e7eb2e]">
              <span>your cart is empty, <br />please add some items.</span>
              <div>
                <Link href={HOME_ROUTE} className="text-sm font-medium tracking-wide hover:text-primary text-slate-900 dark:text-slate-400  cursor-pointer">Visit Store</Link>
              </div>
            </div>
          }
          {/* sidebar */}
          <div className="bg-[#ffffff38] dark:bg-[#2e2e2e4a] rounded-md px-4 py-6 h-max shadow-sm border border-gray-200 dark:border-[#e5e7eb2e]">
            <ul className="text-slate-500 font-medium space-y-4">
              ...
              <li className="flex flex-wrap gap-4 text-sm font-semibold text-slate-900 dark:text-slate-500 ">Total <span className="ml-auto">Rs. {finalPrice}</span></li>
            </ul>
            ...
            {products.length != 0
              ...
              :
              <div className="mt-4 space-y-4">
                <Link href={PRODUCTS_ROUTE} className="text-sm px-4 py-2.5 w-full flex justify-center font-medium tracking-wide bg-[#f8fafc82] dark:bg-[#58585882] hover:bg-primary hover:text-white text-slate-900 dark:text-slate-300 border border-gray-300 dark:border-[#e5e7eb2e] rounded-md cursor-pointer">Continue Shopping</Link>
              </div>
            }
